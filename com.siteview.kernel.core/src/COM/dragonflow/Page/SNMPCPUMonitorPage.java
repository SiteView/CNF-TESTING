...
public class SNMPCPUMonitorPage extends COM.dragonflow.Page.CGI {

    private int mibBlkSize;

    public SNMPCPUMonitorPage() {
        mibBlkSize = 1;
    }
    ...
    private void NextLink(String s, String s1,
            ...
            String s5) {
        outputStream.println(getPagePOST("SNMPCPU", ""));
...
//        String s2 = request.getValue("port");
        ...
        outputStream
                .print("<P><CENTER><H2>SNMPCPU</H2></CENTER><P>SNMPCPU Diagnostic page is a tool that will retrieve the next nth OIDs in the specified object using \"GET NEXT\" requests to the host.");
        ...
        if (request.isPost()) {
            for (int i = 0; i < COM.dragonflow.Utils.TextUtils.toInt(s5); i++) {
                String s6 = COM.dragonflow.StandardMonitor.SNMPCPUMonitor
                        .readSNMPValue(s, "SNMPCPU Tools Page", 5, 1, s3,
                                stringbuffer, s1, s2, false, s4);
                String s7 = COM.dragonflow.StandardMonitor.SNMPCPUMonitor
                        .readSNMPValue(s, "SNMPCPU Tools Page", 5, 1, s3,
                                stringbuffer1,
