...
public class SNMPMemoryPage extends COM.dragonflow.Page.CGI {

    private int mibBlkSize;

    public SNMPMemoryPage() {
        mibBlkSize = 1;
    }
    ...
    private void NextLink(String s, String s1,
            ...
            String s5) {
        outputStream.println(getPagePOST("SNMPMemory", ""));
        ...
        outputStream
                .println("<TR><TD ALIGN=RIGHT>Next OID:</TD><TD><input type=text name=nextoid value=\""
                        + s1 + "\" size=32></TD></TR>\n");
        outputStream
                .println("<TR><TD ALIGN=RIGHT>Index:</TD><TD><input type=text name=port value=\""
                        + s3 + "\" size=10></TD></TR>\n");
        outputStream
...
//        String s2 = request.getValue("port");
        ...
        printBodyHeader("SNMPMemory Diagnostic Page");
        outputStream
                .print("<P><CENTER><H2>SNMPMemory</H2></CENTER><P>SNMPMemory Diagnostic page is a tool that will retrieve the next nth OIDs in the specified object using \"GET NEXT\" requests to the host.");
        ...
        if (request.isPost()) {
            for (int i = 0; i < COM.dragonflow.Utils.TextUtils.toInt(s5); i++) {
                String s6 = COM.dragonflow.StandardMonitor.SNMPMemoryMonitor
                        .readSNMPValue(s, "SNMPMemory Tools Page", 5, 1, s3,
                                stringbuffer, s1, s2, false, s4);
                String s7 = COM.dragonflow.StandardMonitor.SNMPMemoryMonitor
                        .readSNMPValue(s, "SNMPMemory Tools Page", 5, 1, s3,
                                stringbuffer1,
