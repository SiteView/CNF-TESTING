...
public class SNMPMemoryMonitor extends AtomicMonitor{
    ...
    public SNMPMemoryMonitor() {
        labelsCache = null;
    }

    public String getTestURL() {
        String s = "/SiteView/cgi/go.exe/SiteView?page=SNMPMemory&";
    ...
    public static String readSNMPValue(String s, String s1, int i, int j, String s2, StringBuffer stringbuffer, String s3, String s4, boolean flag, String s5, String s6, String s7) {
    	SNMPMemoryMonitor spMonitor=new SNMPMemoryMonitor();
    	snmp=spMonitor.createSnmp(s, 161, s2,i, j);
    	...
    	for(int list_i=0;list_i<list.size();list_i++)
    	{
    		...
    		{
	    		sb.append("memory"+str[0]+" Utilization:");
	    		sb.append(str[1]+"%\t");
	    	}
    	}
    	 return sb.toString();
    ...
    String getOIDTitle(String s) {
        ...
        for (Enumeration enumeration = siteviewgroup.getMultipleValues("_snmpmemoryMonitor"); enumeration.hasMoreElements();) {
    ...
    public Vector getScalarValues(ScalarProperty scalarproperty, HTTPRequest httprequest, CGI cgi) throws SiteViewException {
        if (scalarproperty == pOID || scalarproperty == p2OID || scalarproperty == pPercentageBase) {
            ...
            SiteViewGroup siteviewgroup = SiteViewGroup.currentSiteView();
            Enumeration enumeration = siteviewgroup.getMultipleValues("_snmpmemoryMonitor");
            while (enumeration.hasMoreElements()) {
    ...
    static {
        ...
        if (hasSecondOID) {
            ...
            addProperties("COM.dragonflow.StandardMonitor.SNMPMemoryMonitor", astringproperty);
        } else {
            ...
            addProperties("COM.dragonflow.StandardMonitor.SNMPMemoryMonitor", astringproperty1);
        }
        addClassElement("COM.dragonflow.StandardMonitor.SNMPMemoryMonitor", Rule.stringToClassifier("status != 'ok'\terror"));
        addClassElement("COM.dragonflow.StandardMonitor.SNMPMemoryMonitor", Rule.stringToClassifier("status == 'ok'\tgood"));
        setClassProperty("COM.dragonflow.StandardMonitor.SNMPMemoryMonitor", "description", "Returns the value of an SNMP get.");
        setClassProperty("COM.dragonflow.StandardMonitor.SNMPMemoryMonitor", "help", "SNMPMon.htm");
        setClassProperty("COM.dragonflow.StandardMonitor.SNMPMemoryMonitor", "title", "SNMPMemory");
        setClassProperty("COM.dragonflow.StandardMonitor.SNMPMemoryMonitor", "class", "SNMPMemoryMonitor");
        setClassProperty("COM.dragonflow.StandardMonitor.SNMPMemoryMonitor", "target", "_host");
        setClassProperty("COM.dragonflow.StandardMonitor.SNMPMemoryMonitor", "topazName", "SNMPMemory");
        setClassProperty("COM.dragonflow.StandardMonitor.SNMPMemoryMonitor", "toolName", "SNMPMemory");
        setClassProperty("COM.dragonflow.StandardMonitor.SNMPMemoryMonitor", "toolDescription", "Walks an SNMP mib.");
        setClassProperty("COM.dragonflow.StandardMonitor.SNMPMemoryMonitor", "topazType", "System Resources");
        try {
            HashMap hashmap1 = MasterConfig.getMasterConfig();
            int i = TextUtils.toInt(TextUtils.getValue(hashmap1, "_snmpmemoryMonitorMaximum"));
            if (i <= 0) {
                i = 10;
            }
            if (snmpLock == null) {
