...
//import rcpdev.contact.ui.jobs.SaveServerJob;

public class ServerListPreferencePage extends PreferencePage implements
	...
	public ServerListPreferencePage() {

	}

	public ServerListPreferencePage(String title) {
		super(title);
	}

	public ServerListPreferencePage(String title, ImageDescriptor image) {
		super(title, image);
	...
	protected Control createContents(Composite parent) {
		...
		listViewer.setContentProvider(new ServerContentProvider());
		listViewer.setLabelProvider(new ServerLabelProvider());
		...
		editButton.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(final SelectionEvent e) {
				...
				if (dialog.open() == Dialog.OK) {
					String value = dialog.getBean().getValue();
					bean.updateServer(oldVal, value);
				}
			}
		});
		editButton
	...
	public boolean performOk() {
		SaveServerJob job = new SaveServerJob();
		job.setNewServers(bean.getServers());
		job.schedule();
		try {
			job.join();
